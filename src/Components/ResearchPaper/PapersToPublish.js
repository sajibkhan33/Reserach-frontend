...
import styles from './PapersToPublish.module.css';
...
const PapersToPublish = () => {
  ...
  useEffect(() => {
    const fetchResearchPapers = async () => {
      try {
        const response = await fetch("http://127.0.0.1:8000/api/publish-research-data/", {
          method: 'GET',
          headers: {
  ...
  const markAsPublished = async (id) => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/researchpapers/${id}/mark_as_publish/`, {
        method: 'PUT',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
      ...
      setResearchPapers((prevPapers) =>
        prevPapers.map((paper) => (paper.id === id ? { ...paper, status: 'published' } : paper))
      );
    } catch (error) {
                ...
                <div className={styles.card}>
                  <div className={styles['card-body']}>
                    ...
                    <div className={styles['view-pdf-button']}>
                      ...
                      <button
                        onClick={() => markAsPublished(researchPaper.id)}
                        style={{ textDecoration: 'none', color: 'white', width: '100%' }}
                        className={styles['link-comments']}
                      >
                        <FontAwesomeIcon icon={faUpload} size="md" className={styles.icon} />
                        {' '}
                        Publish
                      </button>
                    </div>
...
export default PapersToPublish;
